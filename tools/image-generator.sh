...
OPTIONS
    ...
    -c             a .c of a .fbp file that will be compiled and installed
                   in the generated image and started at boot time.
...
args=0
sysroot_usr="../build/soletta_sysroot/usr"
product=""
while getopts "x:c:h" o; do
    case "${o}" in
        x)
            ...
            args=$[$args + 2] ;;
        c)
            IFS=, read -ra file <<<"${OPTARG}"
            product=`basename $file | cut -d . -f1`
            ext=`basename $file | cut -d . -f2`

            if [ $ext == "fbp" ]; then
                $sysroot_usr/bin/sol-fbp-generator -j $sysroot_usr/share/soletta/flow/descriptions/ "$file" "$product.c"
            elif [ $ext != "c" ]; then
                printf "Script accepts as input either fbp or c file\n"
                usage 2; exit
            fi

            gcc -o  $product -g "$product.c" -I$sysroot_usr/include/soletta -L$sysroot_usr/lib/ -lsoletta -L$sysroot_usr/lib/soletta/modules/flow/ -l:calamari.so

            hooks+=("cp $sysroot_usr/lib/soletta/modules/flow/*.so \$ROOTFS/usr/lib")
            hooks+=("cp $sysroot_usr/lib/libsoletta.so \$ROOTFS/usr/lib")
            hooks+=("cp ./$product \$ROOTFS/usr/bin")
            args=$[$args + 2] ;;
        h) usage 1; exit 0;;
        \?) usage 2; exit 1;;
    esac
...
cat > $ROOTFS/etc/systemd/system/test.service <<EOF
[Unit]
Description=test

[Service]
ExecStart=/usr/bin/$product
...
# soletta app
systemctl --root=$ROOTFS enable test.service
